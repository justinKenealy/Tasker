...
const renderLeftPane = async (user) => {
  ...
  axios.get(`/api/projects/${user.id}`).then((response) => {
    const { data } = response
    for (let project of data) {
      ...
      projectName.addEventListener('click', async function () {
        ...
        axios.get('/api/session').then(({ data }) => {
          renderTasks(tasks.data, project.name, project.id, data.user)
        })
      })
      editButton.addEventListener('click', async function () {
        const tasks = await axios.get('/api/tasks/project/' + project.id)
        axios.get('/api/session').then(({ data }) => {
          renderEditDeleteProjectForm(project, data.user, tasks.data)
        })
      })
    }
  })
