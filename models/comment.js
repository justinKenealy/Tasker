...
const getCommentsByUserId = (id) => {
  return db.query('SELECT * FROM comments WHERE user_id = $1;', [id]).then((result) => result.rows)
}

const getCommentsByTaskId = (taskId) => {
  return db
    .query(
      'SELECT comments.*, users.user_name FROM comments JOIN users ON comments.user_id = users.id WHERE comments.task_id = $1;',
      [taskId]
    )
...
const deleteCommentById = (id) => {
  return db.query('DELETE FROM comments WHERE id = $1', [id]).then(() => {})
}
